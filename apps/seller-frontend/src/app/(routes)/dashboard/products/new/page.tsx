...
const ProductCreatePage = (props: Props) => {
  ...
  const { data, isLoading: isCategoryLoading, isError } = useQuery({
    queryKey: ['categories'],
    queryFn: async () => {
      try {
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <div className="flex items-center text-xs sm:text-sm text-slate-500">
              <span className="cursor-pointer transition hover:text-slate-700">Dashboard</span>
              <ChevronRight size={14} className="mx-2 opacity-60" />
              <span className="cursor-pointer transition hover:text-slate-700">Products</span>
              <ChevronRight size={14} className="mx-2 opacity-60" />
        ...
        <section className="space-y-6">
          <div className="rounded-3xl border border-slate-200 bg-white/95 p-6 shadow-sm backdrop-blur">
            ...
            <div className="mt-6 grid gap-5 sm:grid-cols-2">
              <div className="sm:col-span-2">
                ...
                />
                {errors.productTitle && (
                  <p className="mt-1 text-xs text-red-500">{errors.productTitle.message}</p>
                )}
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="mt-2 w-full rounded-2xl border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 placeholder:text-slate-400 transition focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/40"
                />
                <p className="mt-1 text-xs text-slate-500">Separate tags with commas for better discovery.</p>
                {errors.tags && (
              ...
              <div>
                ...
                />
                {errors.warranty && (
                  <p className="mt-1 text-xs text-red-500">{errors.warranty.message}</p>
                )}
              </div>
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-slate-500">Use lowercase, numbers, and hyphens only.</p>
              </div>
              ...
              <div className="sm:col-span-2">
                ...
                />
                <p className="mt-1 text-xs text-slate-500">Optional. Supports YouTube and Vimeo embeds.</p>
                {errors.videoUrl && (
          ...
          <div className="rounded-3xl border border-slate-200 bg-white/95 p-6 shadow-sm backdrop-blur">
            ...
            <div className="mt-6 grid gap-5 sm:grid-cols-2">
              <div>
                ...
                />
                {errors.regularPrice && (
                  <p className="mt-1 text-xs text-red-500">{errors.regularPrice.message}</p>
                )}
              </div>
                  ...
                  {...register('salePrice', {
                    ...
                    validate: {
                      ...
                      compareWithRegular: (value) => {
                        ...
                        if (!Number.isFinite(sale) || !Number.isFinite(regular)) {
                          return true;
                        }
                        return sale <= regular || 'Sale price cannot exceed regular price';
                      },
                    },
                  })}
                  className={`mt-2 w-full rounded-2xl border bg-white px-3 py-2 text-sm text-slate-900 placeholder:text-slate-400 transition focus:outline-none focus:ring-2 focus:ring-blue-500/40 ${
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-slate-500">Communicate fulfilment options transparently.</p>
                {errors.casn_on_delivery && (
                  <p className="mt-1 text-xs text-red-500">{errors.casn_on_delivery.message}</p>
                )}
              </div>

              <div className="sm:col-span-2">
                <label className="text-xs font-semibold text-slate-900">Discount codes</label>
                <input
                  type="text"
                  placeholder="SPRING20, VIPACCESS"
                  {...register('discountCodes')}
                  className="mt-2 w-full rounded-2xl border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 placeholder:text-slate-400 transition focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/40"
                />
                <p className="mt-1 text-xs text-slate-500">
                  Optional. Separate multiple codes with commas to track campaign performance.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-3xl border border-slate-200 bg-white/95 p-6 shadow-sm backdrop-blur">
            ...
            <div className="mt-6 grid gap-5 sm:grid-cols-2">
              <div>
                ...
                {isError && (
                  <p className="mt-1 text-xs text-red-500">Unable to load categories. Please retry.</p>
                )}
              </div>
              ...
              <div>
                ...
                {errors.subCategory && (
                  <p className="mt-1 text-xs text-red-500">{errors.subCategory.message}</p>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="rounded-3xl border border-slate-200 bg-white/95 p-6 shadow-sm backdrop-blur">
            ...
            <div className="mt-6 space-y-6">
              <div>
                <div className="flex items-center justify-between">
                  <label className="text-xs font-semibold text-slate-900">Sizes</label>
                  <span className="text-xs text-slate-500">Select all sizes currently in stock.</span>
                </div>
                <div className="mt-3">
                  <SizeSelector
              ...
              <div>
                <div className="flex items-center justify-between">
                  <label className="text-xs font-semibold text-slate-900">Colors</label>
                  <span className="text-xs text-slate-500">Highlight signature shades and limited drops.</span>
                </div>
                <div className="mt-3">
                  <ColorSelector
          ...
          <div className="rounded-3xl border border-slate-200 bg-white/95 p-6 shadow-sm backdrop-blur">
            ...
            >
              ...
              <span className="rounded-2xl border border-slate-200 bg-slate-50 p-2 text-slate-500 transition hover:bg-slate-100">
                <ChevronDown
                  size={18}
                  className={`transition-transform duration-300 ${showSpecifications ? 'rotate-180' : ''}`}
                />
              </span>
            </button>
            ...
            {!showSpecifications && (
              <p className="mt-4 text-xs text-slate-500">
                Add storage capacity, material composition, certifications, and more tailored specs.
              </p>
            )}
          </div>
        </section>
        ...
        <aside className="space-y-6">
            ...
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="mt-2 text-xs text-slate-500">
                  Upload up to 8 high-resolution images. Primary image appears in search and listings.
                </p>
              </div>
            </div>
          ...
          <div className="rounded-3xl border border-slate-200 bg-white/95 p-6 shadow-sm backdrop-blur">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-slate-900">Listing guidelines</p>
                <p className="mt-1 text-xs text-slate-500">
                  Use neutral backgrounds, include lifestyle photos, and keep file sizes under 5MB for fast loading.
                </p>
              </div>
            </div>
            <ul className="mt-4 space-y-3 text-xs text-slate-500">
              ...
              <li>• Ensure sale price is lower than regular price to display badges.</li>
            </ul>
          </div>
          ...
          <div className="rounded-3xl border border-slate-200 bg-slate-900 p-6 text-white shadow-sm">
            ...
            <p className="mt-1 text-sm text-slate-200/80">
              Review your details before publishing. You can always edit after publishing.
            </p>
