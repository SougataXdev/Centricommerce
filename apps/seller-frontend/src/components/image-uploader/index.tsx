...
const ImageUploader = ({
  ...
  return (
    ...
    >
      ...
      {imagePreview ?(
        <>
        <button type='button' onClick={()=>onRemove?.(index!)} className='absolute top-2 right-3 text-red-500 bg-red-100 p-1 rounded-md'><Trash2/></button>
        <button className='absolute top-2 right-12 bg-blue-300 p-1 rounded-md '><WandSparkles/></button>
        </>
      ):(
        <label
